...
import { numbers } from "../../utils/number-circuitron";
import { Button } from "@material-ui/core";

import { setSelectedQuestionCircuitron } from "../../redux/circuitron-question/question.actions";
import {
...
const Grid = ({
  ...
  questionsCircuitron,
  selectedQuestionNumberCircuitron,
  mounted,
}) => {
  return (
    <Wrapper>
          ...
          <div className="number-box">
            ...
            {numbers.map((number) => {
              ...
              console.log(
                ...
                selectedQuestionNumberCircuitron === num - 1
              );
              return (
                <StyledButton
                  className="small-rectangle"
                  key={id}
                  onClick={() => setSelectedQuestionCircuitron(num)}
                  marked={mounted ? questionsCircuitron[num - 1].marked : false}
                  selected={selectedQuestionNumberCircuitron === num - 1}
                  style={{}}
                >
                  <h4>{num}</h4>
                </StyledButton>
...
const mapDispatchToProps = (dispatch) => ({
  setSelectedQuestionCircuitron: (question) => dispatch(setSelectedQuestionCircuitron(question)),
});
